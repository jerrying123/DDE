...
if __name__== "__main__":
	N_states = 2
	n_c = 5
	datasizes = [500,1500,2500,5000,10000,25000]
	for dsize in datasizes:
		...
		qr = pd.read_csv('./models/qr' + str(n_c) + '.csv',header = None)
		A_qr = np.array(qr.values)

		base = pd.read_csv('./models/base' + str(n_c) + '.csv',header = None)
		...
		np.savetxt('./errors/edmd_error'+str(dsize)+'.csv',base_sse, delimiter = ",")
		np.savetxt('./errors/rdmd_error'+str(dsize)+'.csv',qr_sse, delimiter = ",")
